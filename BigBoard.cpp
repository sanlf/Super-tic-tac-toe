...
BigBoard(Coordinate p0, Coordinate p1)
{
    ...
    for(int i = 0; i < 9; ++i){
        boards[i].p0 = Coordinate(this->p0.x + (i % 3) * sbwidth,
                                 this->p0.y + (i / 3) * sbheight);
        boards[i].p1 = Coordinate(this->p1.x + (i%3 + 1) * sbwidth,
                                 this->p1.y + (i/3 + 1) * sbheight);
    }
}
